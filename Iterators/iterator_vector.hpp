...
{
  template < class T >
    class iterator_vector : public ft::iterator<random_access_iterator_tag, T>
  {
    public:
    ...
    public:
      // ========>> Default Constructor <<========
      iterator_vector() : _p(nullptr)                                         { }

      // ========>> Copy Constructor <<========
      template < class U >
        iterator_vector ( const iterator_vector<U> & copy ) : _p(copy.base()) { }

      // ========>> Custom Constructor <<========
      iterator_vector ( const pointer p ) : _p(p)                             { }
      iterator_vector ( const reference r ) : _p(&r)                          { }

      // ========>> Copy Assignment Operator <<========
      template < class U >
        iterator_vector &operator= ( const iterator_vector<U> & copy )        { _p = copy.base(); return (*this); }
      ...
      iterator_vector        operator- ( const difference_type & n ) const                      { return (iterator_vector(_p - n)); }
      ...
      iterator_vector &      operator-= ( const difference_type & n )                           { _p -= n; return (*this); }
      iterator_vector &      operator+= ( const difference_type & n )                           { _p += n; return (*this); }
